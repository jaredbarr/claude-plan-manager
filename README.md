...
```
/plugin marketplace add <owner>/claude-plan-manager
/plugin install plan-manager@<marketplace-name>
```

### Local dev install

```
/plugin marketplace add /path/to/claude-plan-manager
/plugin install plan-manager@<marketplace-name>
```

Replace `<owner>` with the GitHub owner of your fork and `<marketplace-name>` with the `name` field from `.claude-plugin/marketplace.json`.
...
```
# In ~/projects/my-app (a git repo)
~/.claude/plans/nested-coalescing-widget.md
                  ↓ ExitPlanMode fires
~/.claude/plans/2026-04-16-1430-my-app-platform-hosting-comparison.md
